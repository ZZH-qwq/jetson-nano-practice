...
from zzhmodel import MODELRESH, MODELRESW
import numpy as np
...
def build_data(root="./data/cifar-100-python", is_train=True):
    ...
    if is_train:
        train_data, train_labels = preprocess(
            train_dict[b"data"], train_dict[b"fine_labels"])
    else:
        test_data, test_labels = preprocess(
            test_dict[b"data"], test_dict[b"fine_labels"])
