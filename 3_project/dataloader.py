...
class ASRDataset(Dataset):
    def __init__(
        ...
        self.samples = [(wav_paths[i], transcripts[i], wav_lengths[i]) for i in range(
            len(wav_paths)) if wav_lengths[i] <= max_wav_length]
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for sample in self.minibatches[index]:
            ...
            # rescale to int16 for kaldi compatibility
            wav = wav * (1 << 15)
            ...
            mfccs = dct(S_db, type=2, axis=0, norm='ortho')[:80]
            fbanks.append(torch.from_numpy(mfccs.T).float())
     # for sample in self.minibatches[index]:
     #     path, transcript, length = sample
     ...
     #     fb = librosa.feature.melspectrogram(
     #         y=wav, sr=sr, n_mels=80)  # (seq_len, 80)
     #     fbanks.append(torch.from_numpy(fb.T).float())
    # load tokens
        ys_in = []
        ys_out = []
        for sample in self.minibatches[index]:
